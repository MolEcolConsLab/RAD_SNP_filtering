...
1) minor allele frequency (we can filter for MAF and/or MAC in Stacks or vcftools, so probably don't need this here as well)
2) % contribution from forward and reverse read (?)
...
file_directory <- "../data/"
file_prefix <- "populations.snps.recode"
...
## Chunk 8: Evaluate heterozygosity and HWE
...
#Can edit below if wanting to only filter for LD or HWE
 if(filter.HWE.LD == "yes"){
  fix_tidy_filt_final <- fix_tidy_filt1 %>%
    ...
    anti_join(locus_HWE_remove, by = c("ChromKey", "POS")) #HWE
} else {
  fix_tidy_filt_final <- fix_tidy_filt1
}
