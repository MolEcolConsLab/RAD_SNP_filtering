...
2) allele balance (already addressed with HDPlot, I think)
...
source("~/R/R_working_dir/Cownose_PopConn/03_snp_filtering/scripts/ggplot.R")
source("~/R/R_working_dir/Cownose_PopConn/03_snp_filtering/scripts/VCFfilterstats.R")
source("~/R/R_working_dir/Cownose_PopConn/03_snp_filtering/scripts/xtrafunctions.R")
source("~/R/R_working_dir/Cownose_PopConn/03_snp_filtering/scripts/plotstats.R")
source("~/R/R_working_dir/Cownose_PopConn/03_snp_filtering/scripts/filterSNPs.R")
...
The steps summarized above are split into chunks. An accompanying HackMD can be found here: https://hackmd.io/io3eZlAyRGusdqKY3222gA?view. A summary of what each chunk does, as well as any required input files, are just above each chunk and reiterated in the HackMD. Most of the time, the required input files will just be the output of the previous chunks. Any parameters required in a chunk will be specified at the top.
...
1. a folder called "data" containing the following files, all of which should have been output by vcftools into the "09_vcftools_out" folder in the previous step:
  - a .vcf file *less than 1GB in size* containing all variants and samples output by vcftools. This will usually end with ".recode.vcf"
...
setwd("~/R/R_working_dir/")
# load stats files - O'Leary ----
file_directory <- "~/R/R_working_dir/"
file_prefix <- "CNR_raw"
popmap_file <- "CNR_popmap"
...
Step 2: based on the plot in the plot window, decide on the degree of data missingness that constitutes calling an individual as "failed". These individuals will be filtered before running any additional calculations.
...
#minor_allele_indv <- 0 #Number of individuals the minor allele must be present in #JDS: remove?
...
*Presently, the script removes loci that have a mean depth (across individuals) that is > 2 standard deviations from the mean.*
...
fix_tidy_filt_final <- fix_tidy_filt2 #%>% semi_join(LD_loci_to_keep, by = c("ChromKey", "CHROM", "POS"))
...
gt_tidy_filt_final <- gt_tidy_filt2 %>% 
  ...
  dplyr::select(ChromKey, CHROM, POS, Indiv, gt_AD, gt_DP, gt_HQ, gt_GL, gt_GQ, gt_GT, gt_GT_alleles, allele_A, allele_B, zygosity, REF, ALT)
...
vcf_int <- vcfR::read.vcfR("radiator_CNR_filtered_vcf_05.24.2021.vcf")
