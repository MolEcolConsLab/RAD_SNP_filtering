...
# #Checked 05/06/2021
calc_missing_indv <- function(gt_tidy_fltr){
...
#Locus filtering
##Extract loci that are missing across samples
###Checked against output from VCFTools and it works!
####Make dataframe of loci with a high percentage of missing data

#Checked 05/06/2021
calc_missing_loci <- function(gt_tidy_fltr){
...
#Checked 05/06/2021
filter_indv <- function(gt_tidy_fltr){

  gt_tidy_filt1 <- gt_tidy_fltr %>% semi_join(keep_loci, by = c("ChromKey", "POS")) %>% #Keep only loci that passed filter thresholds
...
filter_loci <- function(gt_tidy_filt1){
...
return(fix_tidy_filt1)
}

#Remove invariant "SNPs" from genotype dataframe as well
#Checked 05/06/2021
merge_tables <- function(gt_tidy_filt1){
...
#Checked 05/06/2021
calc_het <- function(gt_tidy_filt2){
